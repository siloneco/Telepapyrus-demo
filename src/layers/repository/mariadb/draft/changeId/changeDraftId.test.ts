...
describe('changeDraftId', () => {
  ...
  it('changes draft id correctly', async () => {
    ...
    expect(await changeDraftId(user, id, `${id}-new`)).toMatchObject({
      success: true,
    })
  })
  ...
  it('rejects when the draft is not exists', async () => {
    ...
    expect(await changeDraftId(user, id, 'tmp-test-new')).toMatchObject({
      success: false,
      error: {
        id: 'not-exists',
      },
  ...
  it('rejects when 2 or more drafts affected', async () => {
    ...
    expect(await changeDraftId(user, baseId, `${baseId}-new`)).toMatchObject({
      success: false,
      error: {
  ...
  it('rejects when new article id already occupied', async () => {
    ...
    expect(
      await changeDraftId(user, `${baseId}-1`, `${baseId}-2`),
    ).toMatchObject({
      success: false,
      error: {
        id: 'already-exists',
  ...
  it('rejects when new id is too long', async () => {
    ...
    await saveDraft(user, { ...baseData, id: baseId })

    expect(
      await changeDraftId(user, baseId, baseId + longString),
    ).toMatchObject({
      success: false,
      error: {
        id: 'invalid-data',
